...
export default function SocialAccountsPage() {
  ...
  // Uygulama ayarları formu (platform slug → açık/kapalı + alanlar)
            ...
            <Card key={p.slug} className="p-5">
              <div className="flex items-start justify-between gap-3">
                ...
                <Badge className={connected ? 'bg-emerald-500/20 text-emerald-300' : 'bg-white/10 text-white/50'}>
                  {connected ? 'Bağlı' : p.available ? (configured ? 'Hazır' : 'Yapılandırma gerekli') : 'Yakında'}
                </Badge>
              </div>
                ...
                <div className="mt-4 space-y-3 rounded-lg border border-white/10 bg-white/5 p-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[11px] text-white/35">
                      Bu URI'yi {p.displayName} app ayarlarındaki geçerli OAuth redirect listesine birebir ekleyin.
                    </p>
                  </div>
